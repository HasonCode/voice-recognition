...
everything said after it until there is a pause in speech, and feed that
response to a callback (print, file, or e.g. Llama).

Uses the same live pipeline as pipeline_live_test; the wake-word logic runs
on the streamed transcript.

Usage:
  python hey_jetson_listener.py
  python hey_jetson_listener.py --output responses.txt
  python hey_jetson_listener.py --pause 3.0 --similarity 0.8
  python hey_jetson_listener.py --device 1
"""
...
from voice_recognition.wakeword import HeyJetsonListener
