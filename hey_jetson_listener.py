...
LLM_SYSTEM_PROMPT = (
    "You are a helpful assistant. Answer in 1 to 3 short sentences, accurate and concise. "
    "Do not invent what the user asked. If the request is unclear, ask one short clarification."
)
LLM_MAX_TOKENS = 32
LLM_N_CTX = 256
LLM_N_BATCH = 32
...
def main(
    ...
    def run_with_asr_paused(job) -> None:
    ...
    def shorten_answer(text: str, max_sentences: int = 3, max_chars: int = 240) -> str:
    ...
    def on_response(response: str):
        ...
        if not normalized:
            return
        now = time.time()
        if normalized == last_question_sig[0] and now - last_question_ts[0] < 4.0:
            return
        last_question_sig[0] = normalized
    ...
    def on_wake():
        ...
        run_with_asr_paused(intro_then_prompt)
    ...
    def on_display(s):
        ...
        accumulated_transcript[0] = merge_display_into_transcript(accumulated_transcript[0], s_norm)
